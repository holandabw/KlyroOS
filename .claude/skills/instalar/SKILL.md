...
name: instalar
description: >
  Instala o KlyroOS no negócio do usuário. Entrevista sobre empresa, tom de voz,
  ...
  pedir explicitamente "rodar /instalar", "instalar o KlyroOS", "primeiro setup".
---

# /instalar — Instalação inicial do KlyroOS
...
Conferir o nome da pasta atual (`basename "$(pwd)"`). Se for `klyroos`, `KlyroOS`, `KlyroOS-main`, `klyroos-main` ou variação genérica:

> "Notei que a pasta atual ainda tem nome genérico ('<nome-atual>'). O ideal é a pasta ter o nome do seu negócio, não 'KlyroOS'. Quando terminarmos o setup, te lembro de renomear (é rápido — fechar VS Code, renomear a pasta no Finder/Explorer, abrir de novo). Bora seguir?"
...
> "Pronto. O KlyroOS já te conhece.
>
